...
def main(scenario, demand_data, case, fixed_demand=1):
    ...
    for count, demand_factor in enumerate(demand_factors):
        file_name = '{a}_sc{b}_df{c}_fd{d}'.format(a=year, b=scenario, c=demand_factor, d=fixed_demand)
        if count == 0:
...
if __name__ == '__main__':
    year = 2050
    demand_data = 'fuel_demand_port_{a}_1000km.csv'.format(a=year)
    case = 'Fuel_tons_route_future'
    fixed_demand = 0.5
    main('4.5', demand_data, case, fixed_demand=fixed_demand)
